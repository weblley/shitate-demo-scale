...
- 右下の「Aa」ボタン → モーダルで比率 / 小さな画面での比率 / 基準サイズ / 丸め / 小さな文字の数値指定を変更すると、ページの `--st-*` トークンを即時に上書き
- 変更は訪問者のブラウザ（localStorage）にだけ記憶。**サーバー側には何も保存しない**
- 「テーマの設定に戻す」でサイトのカスタマイザー値に復帰
- **ログイン中でテーマ編集権限（`edit_theme_options`）があるユーザーにだけ「テーマの設定として保存」ボタン**が出る。押すとモーダルの値がテーマのカスタマイザー設定に書き込まれ、全訪問者に適用される（REST `sds/v1/theme-scale`、nonce＋権限チェック）
